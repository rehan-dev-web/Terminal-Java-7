...
public class SafActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Gak perlu layout XML, langsung eksekusi aja

        // Perintah ke Android: "Woi, buka folder picker mode TREE!"
        Intent intent = new Intent(Intent.ACTION_OPEN_DOCUMENT_TREE);
        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION 
                        | Intent.FLAG_GRANT_WRITE_URI_PERMISSION 
                        | Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION); // Penting biar permanen
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_SAF && resultCode == Activity.RESULT_OK) {
            if (data != null) {
                ...
                // 1. KUNCI IZINNYA (Biar besok-besok gak usah minta lagi)
                // Ini langkah paling krusial buat Android 10+
                try {
                    final int takeFlags = data.getFlags() & (Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
                    getContentResolver().takePersistableUriPermission(treeUri, takeFlags);

                    // 2. Simpen URI root ke Memory (Opsional, buat tanda aja)
        ...
        // Tugas selesai, tutup activity & balik ke terminal
        finish();
    }
}
