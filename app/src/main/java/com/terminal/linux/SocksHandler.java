...
import java.lang.reflect.Method; // Wajib import ini
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    // === JURUS REFLECTION (BYPASS AIDE ERROR) ===
                    // Kita panggil metodenya pake "Nama String" biar AIDE ga protes
                    try {
                        // Coba panggil: setPortForwardingD(int port)
                        Method method = session.getClass().getMethod("setPortForwardingD", int.class);
                        method.invoke(session, localPort);
                    } catch (Exception e1) {
                        // Kalo gagal, coba panggil: setPortForwardingD(String bind, int port)
                        try {
