...
public class FileManager {
    ...
    // List Ekstensi (Gabungan dari script lu)
    ...
    // ==========================================
    // 1. LOGIC RENAME (Porting dari rename.bsh)
    // ==========================================
    private void handleRename(String[] args) {
        // Parse Argumen manual ala Java
        String targetPath = "";
        String pattern = "";
        String type = "";
        boolean recursive = false;

        // Simple parser (file rename <dir> -n <pola> -f <tipe>)
        try {
            targetPath = args[2];
    ...
    private int countNB = 1; // Counter global per folder

    private void processRenameScan(File dir, String pattern, String type, boolean recursive) {
        countNB = 1; // Reset counter tiap masuk folder baru
        File[] files = dir.listFiles();
        if (files == null) return;
        Arrays.sort(files); // Biar urut
    ...
    // ==========================================
    // 2. LOGIC DESTINATION (Porting dari destination.bsh)
    // ==========================================
    private void handleDestination(String[] args) {
        ...
        try {
            // Format: file destination [src] -t [tipe] -d [dest]
            srcPath = args[2];
            for (int i = 0; i < args.length; i++) {
    ...
    private void processMoveRecursive(File dir, File destDir, String type) {
        ...
        for (File f : files) {
            if (f.isDirectory()) {
                // Jangan scan folder tujuan biar ga loop
                ...
                // Logic Filter sama kayak rename
    ...
    private void moveFileSafe(File source, File destDir) {
        ...
        // Auto Rename Duplicate (file_1.jpg)
        int c = 1;
        while (target.exists()) {
